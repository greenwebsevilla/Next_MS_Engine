...
IF LEVEL_NUM = 0     'SI ESTAMOS EN EL MAPA O NIVEL 0'
    ...
    'DETECCIÓN DE ZONA DEL MAPA DEL NIVEL 0, IMPRIMIMOS TEXTOS'
    IF PLAYER_IN_ZONE(6,6,7,6) AND PRESS_UP
        DIALOG (1+LANGUAGE*100)
    END IF

    'DETECCIÓN DE ZONA DEL MAPA DEL NIVEL 0, CAMBIAMOS LA MÚSICA E IMPRIMIMOS TEXTOS'
    IF PLAYER_IN_ZONE(18,6,19,6) AND PRESS_UP
        DIALOG (2+LANGUAGE*100)
        MUSIC = 3
    END IF
