...
Versión: 1.2.1
---------------------------------------------
- Añadida exporatación de capas de objetos para enemigos.
"""
...
TEXTS = {
    "es": {
        ...
        "help_text": (
            "Instrucciones:\n\n"
            "- Este programa genera a partir de mapas creados con Tiled (tmx) ficheros binarios con la información de los tiles que componen los mapas: 'nombre_tiles.bin'. Procesa todos los mapas que encuentre en la carpeta de origen.\n"
            "Si el mapa contiene una capa de objetos con la información de los enemigos (capa enemies), se generará también un binario con esa información: 'nombre_enemies.bin'\n\n"
...
def process_tmx_folder(source_folder, bin_folder, map_folder=None, generate_map=False, lang="es"):
    ...
    TILE_SIZE = 16  # <-- ajusta si tu engine usa otro tamaño de tile
    ...
    for tmx_file in tmx_files:
        ...
        if markers_layer is not None:
            for mobj in markers_layer.findall("object"):
                try:
                    mid = int(mobj.get("id"))
                    # x,y vienen en píxeles; convertimos a coordenadas de tile
        ...
        for layer in root.findall("layer"):
            ...
            if layer_name == "_temp_markers":
                continue  # ❌ Ignorar esta capa especial
        ...
        #    Primer byte del archivo = número total de enemigos
        # --------------------------------------------------
                ...
                for obj in enemies_layer.findall("object"):
                    ...
                    if props_elem is not None:
                        for p in props_elem.findall("property"):
                            ...
                            # intentamos primero 'value'; si no está, usamos el texto o 0.
                            val = p.get("value")
                            if val is None:
                    ...
                    # Calcular spawnX y spawnY a partir de la posición del objeto enemigo en Tiled
                    # Tiled usa la esquina inferior izquierda (por defecto), así que restamos la altura para obtener la esquina superior izquierda
                    try:
                    ...
                    # Conversión a uint8 con módulo 256 (permite negativos p.ej. velocidades)
                    clean_tuple = tuple(int(v) % 256 for v in data_tuple)

                    f.write(struct.pack("12B", *clean_tuple))
                    enemy_count += 1

                # Escribir el número de enemigos al principio
                f.seek(0)
                f.write(struct.pack("B", enemy_count))

            print(f"👾 Enemies BIN guardado: {enemies_bin_path} ({enemy_count} enemigos)")
